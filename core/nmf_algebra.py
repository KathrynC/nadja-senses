...
def normalize_smell_vector(v: Dict[str, float]) -> Dict[str, float]:
    """Normalize a smell vector so the sum of absolute values is 1.0."""
    total = sum(abs(val) for val in v.values())
    if total < 1e-10:
        return {cat: 0.0 for cat in SMELL_CATEGORIES}
...
def compute_gestalt(activations: Dict[str, float]) -> str:
    ...
    # Top positive activations = primary scent
    primary = [cat for cat, val in sorted_cats if val > 0.001][:2]
    # Active negative = undertones of decay
    ...
    return " ".join(parts).upper() if parts else "NEUTRAL — NO STRONG SCENT"
